...
export async function ensureCommentLinkFormat(
  config: CrawlConfig
): Promise<CrawlConfig> {
  const commentLinkFormatSuggestion =
    config.commentLinkFormat || DEFAULT_COMMENT_LINK_TEMPLATE;

  if (!commentLinkFormatSuggestion) {
    throw new Error(
      `Comment link template ${commentLinkFormatSuggestion} invalid check your settings in ${getConfigPath()}`
    );
  }
  if (
